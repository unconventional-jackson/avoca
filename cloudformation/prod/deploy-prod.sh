...
STEP_03_EC2=1
STEP_04_ECR=0
STEP_05_MIGRATIONS=0
STEP_06_API_SERVICE=0
STEP_07_CALLS_SERVICE=0
STEP_08_S3_CLOUDFRONT=0
STEP_09_FRONTEND_SERVICE=0
...
export $(aws secretsmanager get-secret-value --secret-id /prod/avoca/api-service --query SecretString --output text | jq -r 'to_entries | map("\(.key)=\(.value)") | .[]')
echo "Fetched secret for /prod/avoca/api-service"
...
# If we need paramter overrides, add a .env.prod and --parameter-overrides $(cat .env.prod)
...
if [ "$STEP_01_VPC" -eq 1 ]; then
  ...
  aws cloudformation deploy \
    --profile $AWS_PROFILE \
    --region $AWS_REGION \
    --template-file prod-ue1-avoca-vpc.yaml \
    --stack-name prod-ue1-avoca-vpc \
...
if [ "$STEP_02_SG" -eq 1 ]; then
  ...
  aws cloudformation deploy \
    --profile $AWS_PROFILE \
    --region $AWS_REGION \
    --template-file prod-ue1-avoca-sg.yaml \
    --stack-name prod-ue1-avoca-sg \
    --capabilities CAPABILITY_IAM
    # --parameter-overrides $(cat .env.prod)
...
if [ "$STEP_03_EC2" -eq 1 ]; then
  ...
  aws cloudformation deploy \
    --profile $AWS_PROFILE \
    --region $AWS_REGION \
    --template-file prod-ue1-avoca-ec2.yaml \
    --stack-name prod-ue1-avoca-ec2 \
    --capabilities CAPABILITY_IAM
    # --parameter-overrides $(cat .env.prod)
...
if [ "$STEP_04_ECR" -eq 1 ]; then
  ...
  aws cloudformation deploy \
    --profile $AWS_PROFILE \
    --region $AWS_REGION \
    --template-file prod-ue1-avoca-ecr.yaml \
    --stack-name prod-ec1-avoca-ecr \
    --capabilities CAPABILITY_IAM
    # --parameter-overrides $(cat .env.prod)
...
if [ "$STEP_05_MIGRATIONS" -eq 1 ]; then
  ...
  POSTGRES_HOST=127.0.0.1 POSTGRES_PORT=5432 POSTGRES_USER=$POSTGRES_USER POSTGRES_PASSWORD=$POSTGRES_PASSWORD POSTGRES_DB=$POSTGRES_DB npm --prefix ../../database-service run migration:up:prod
...
if [ "$STEP_06_API_SERVICE" -eq 1 ]; then
  ...
  ECR_REPOSITORY="$AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/prod-ue1-avoca-api-container"
  ...
  IMAGE_NAME=prod-ue1-avoca-api-container
  DOCKERFILE_PATH=../../api-service/Dockerfile.prod
...
if [ "$STEP_07_CALLS_SERVICE" -eq 1 ]; then
  ...
  # e.g., 123456789012.dkr.ecr.us-east-1.amazonaws.com/my-repo
  ECR_REPOSITORY="$AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/prod-ue1-avoca-calls-container"
  echo "ECR Repository: $ECR_REPOSITORY"
  aws ecr get-login-password \
    --region $AWS_REGION | docker login \
    --username AWS \
  ...
  # -t flag is used to tag the image with a name and optionally a tag in the 'name:tag' format
  echo "Building Docker image..."
  IMAGE_NAME=prod-ue1-avoca-calls-container
  DOCKERFILE_PATH=../../calls-service/Dockerfile.prod
...
if [ "$STEP_08_S3_CLOUDFRONT" -eq 1 ]; then
  ...
  aws cloudformation deploy \
    --profile $AWS_PROFILE \
    --region $AWS_REGION \
    --template-file prod-ue1-avoca-frontend.yaml \
    --stack-name prod-ue1-avoca-frontend \
    --capabilities CAPABILITY_IAM
    # --parameter-overrides $(cat .env.prod)
fi
...
if [ "$STEP_09_FRONTEND_SERVICE" -eq 1 ]; then
  ...
  S3_BUCKET="s3://$AWS_ACCOUNT_ID-us-east-1-prod-avoca-web-app"
  DISTRIBUTION_KEY=ProdUe1AvocaCloudFrontDistributionId
